...
```{r}
french <- sf::st_read(here("Week_3_prac", "gadm41_FRA.gpkg"),
                      layer='ADM_ADM_0')

world_cities <- sf::st_read(here('Week_3_prac','World_Cities.csv'))

ssp1 <-terra::rast(here("Week_3_prac", "wc2.1_5m_tmax_ACCESS-CM2_ssp126_2081-2100.tif"))

# ssp1_mean <- mean(ssp1) # why this step?

ssp5 <-terra::rast(here("Week_3_prac", "wc2.1_5m_tmax_ACCESS-CM2_ssp585_2081-2100.tif"))
...
french_cities <- world_cities %>%
  janitor::clean_names()%>%
  dplyr::filter(cntry_name=="France")
...
#### sub
diff_climate_model <- exact_french5 - exact_french1 
```
...
french_cities_simplify <- french_cities[c(1,2,5)] %>%
  as.data.frame()
row.names(french_cities_simplify) <- french_cities_simplify$city_name
french_cities_simplify <- french_cities_simplify[,1:2]

french_cities_simplify$x <- as.numeric(french_cities_simplify$x)
french_cities_simplify$y <- as.numeric(french_cities_simplify$y)

french_city_diff<- terra::extract(diff_climate_model, french_cities_simplify)
...
french_cities_join_ID <- french_cities %>%
...
french_city_diff2 <- french_cities_join_ID %>%
  dplyr::left_join(.,
            french_city_diff,
             by = c("join_id" = "ID"))
...
city_climate_diff <- french_city_diff2 %>%
  dplyr::select(c(,18:29)) %>%
  sf::st_drop_geometry(.) %>%
  dplyr::as_tibble() 
